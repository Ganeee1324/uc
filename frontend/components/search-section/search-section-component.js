class SearchSectionComponent extends HTMLElement {
    constructor() {
        ...
        // Isolated DOM cache for this component
        this.domCache = {};
        ...
        this.shadowRoot.innerHTML = `
            ...
            <div class="search-section has-results" id="searchSection">
                ...
                <div class="documents-grid" id="documentsGrid">
                    <!-- Loading cards will be generated dynamically -->
                </div>
            </div>
            ...
            <div class="filters-panel" id="filtersPanel">
                <div class="filters-content">
                    ...
                    <div class="filters-grid">
                        <!-- Filters content will be dynamically generated -->
                    </div>
                </div>
            </div>
            ...
            <script id="dynamic-script">
                // Script content will be injected dynamically
            </script>
        `;
    }
    ...
    init() {
        ...
        // Set current component as the host for the search-section.js
        window.currentWebComponentHost = this;
        
        // Initialize the modified search-section.js functionality
        if (window.initializeSearchSection) {
            window.initializeSearchSection(this);
        } else {
            console.log('Modified search-section.js is ready to be loaded');
        }
    }

    initDOMCache() {
        // Cache frequently used DOM elements within shadow root
        this.domCache = {
            documentsGrid: this.shadowRoot.getElementById('documentsGrid'),
            searchSection: this.shadowRoot.querySelector('.search-section'),
            documentCountContainer: this.shadowRoot.getElementById('documentCountContainer'),
            documentCount: this.shadowRoot.getElementById('documentCount'),
            searchInput: this.shadowRoot.getElementById('searchInput'),
            filtersBtn: this.shadowRoot.getElementById('filtersBtn'),
            filtersPanel: this.shadowRoot.getElementById('filtersPanel'),
            filtersOverlay: this.shadowRoot.getElementById('filtersOverlay'),
            filtersClose: this.shadowRoot.getElementById('filtersClose'),
            clearAllFilters: this.shadowRoot.getElementById('clearAllFilters'),
            orderBtn: this.shadowRoot.getElementById('orderBtn'),
            activeFiltersDisplay: this.shadowRoot.getElementById('activeFiltersDisplay'),
            filterCount: this.shadowRoot.getElementById('filterCount')
        };
    }

    generateLoadingCards() {
        // Generate initial loading cards
        const loadingCards = Array.from({ length: 8 }, () => this.createLoadingCard()).join('');
        this.domCache.documentsGrid.innerHTML = loadingCards;
    }
    ...
    initializeFilters() {
        // Initialize filter system - will be populated with actual filter logic
        // This is where the filters panel content will be generated
    }
    ...
    setupEventListeners() {
        ...
        this.domCache.searchInput?.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(() => {
                if (this.performSearch) {
                    const aiEnabled = this.shadowRoot.getElementById('toggle')?.checked || false;
                    this.performSearch(e.target.value, aiEnabled);
                }
            }, 300);
        });
        ...
        // AI Search toggle
        const aiToggle = this.shadowRoot.getElementById('toggle');
        aiToggle?.addEventListener('change', (e) => {
            const query = this.domCache.searchInput?.value || '';
            if (query.length >= 2 && this.performSearch) {
                this.performSearch(query, e.target.checked);
            }
        });

        // Filters button
        this.domCache.filtersBtn?.addEventListener('click', () => {
            this.toggleFiltersPanel();
        });

        // Close filters
        this.domCache.filtersClose?.addEventListener('click', () => {
            this.closeFiltersPanel();
        });

        // Filters overlay
        this.domCache.filtersOverlay?.addEventListener('click', () => {
            this.closeFiltersPanel();
        });

        // Clear all filters
        this.domCache.clearAllFilters?.addEventListener('click', () => {
            this.clearAllFilters();
        });

        // Order dropdown
        this.domCache.orderBtn?.addEventListener('click', () => {
            const dropdown = this.shadowRoot.querySelector('.order-dropdown-content');
            dropdown?.classList.toggle('show');
        });

        // Order options
        this.shadowRoot.querySelectorAll('.order-option').forEach(option => {
            option.addEventListener('click', (e) => {
                const orderType = e.currentTarget.getAttribute('data-order');
                this.updateSortOrder(orderType);
            });
        });

        // Document card interactions (event delegation)
        this.shadowRoot.addEventListener('click', (e) => {
            ...
            if (e.target.closest('[data-action="toggle-favorite"]')) {
                ...
                this.toggleFavorite(vetrinaId, element);
            }
            ...
            // Handle reviews
            if (e.target.closest('[data-action="show-reviews"]')) {
                const element = e.target.closest('[data-action="show-reviews"]');
                const vetrinaId = element.getAttribute('data-vetrina-id');
                this.showReviews(vetrinaId);
            }
        });
    }

    // Additional helper methods for the component
    updateSortOrder(orderType) {
        this.state.currentOrder = orderType;
        
        // Update button text
        const orderBtn = this.shadowRoot.querySelector('.order-text');
        const orderOptions = {
            'relevance': 'Rilevanza',
            'reviews': 'Recensioni',
        ...
        if (orderBtn && orderOptions[orderType]) {
            orderBtn.textContent = orderOptions[orderType];
        }
        ...
        // Re-render with new order if we have the function available
        if (this.applyFiltersAndRender) {
            this.applyFiltersAndRender();
        }
    }

    toggleFavorite(vetrinaId, element) {
        ...
        console.log(`Toggling favorite for vetrina ${vetrinaId}`);
    }

    showReviews(vetrinaId) {
        // Open reviews overlay for the specified vetrina
        const reviewsOverlay = this.shadowRoot.getElementById('reviewsOverlay');
        if (reviewsOverlay) {
            reviewsOverlay.classList.add('active');
            // Load reviews data here
            console.log(`Showing reviews for vetrina ${vetrinaId}`);
        }
    }

    toggleFiltersPanel() {
        this.domCache.filtersPanel?.classList.toggle('open');
        this.domCache.filtersOverlay?.classList.toggle('active');
    }

    closeFiltersPanel() {
        this.domCache.filtersPanel?.classList.remove('open');
        this.domCache.filtersOverlay?.classList.remove('active');
    }

    clearAllFilters() {
        this.state.filters = {};
        this.updateFilterDisplay();
        // Clear filters logic will be implemented here
    }

    updateFilterDisplay() {
        const activeCount = Object.keys(this.state.filters).length;
        if (this.domCache.filterCount) {
            this.domCache.filterCount.textContent = activeCount;
        }
    }

    cleanup() {
        // Clean up any timers, listeners, or other resources
        console.log(`Cleaning up search component ${this.componentId}`);
    }

    // Public API methods for external interaction
    search(query) {
        if (this.domCache.searchInput) {
            this.domCache.searchInput.value = query;
    ...
    // Method to inject JavaScript code into the script tag
    injectScriptContent(scriptContent) {
        const scriptElement = this.shadowRoot.getElementById('dynamic-script');
        if (scriptElement) {
    ...
    // Method to add JavaScript code as a string (safer approach)
    addScriptAsString(scriptCode) {
        // This method allows you to add JavaScript code that will be treated as a string
        // and then executed when needed
        const scriptElement = this.shadowRoot.getElementById('dynamic-script');
        if (scriptElement) {
