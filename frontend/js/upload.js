...
async function loadVetrine() {
    try {
        ...
        if (allVetrine.length === 0) {
            ...
            console.error('No vetrine found. You need to create a vetrina first before uploading files.');
            return;
        }
        ...
        console.error('Failed to load vetrine. Please make sure you are logged in and vetrine exist.');
    }
}
...
async function uploadFiles() {
    ...
    if (!vetrinaId || selectedFiles.length === 0) {
        console.error('Please select a vetrina and files to upload');
        return;
    }
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error(`Failed to upload ${file.name}: ${error.msg}`);
            }
        } catch (error) {
