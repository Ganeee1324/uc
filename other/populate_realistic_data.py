...
def populate_realistic_database():
    """Populate database with realistic vetrine and files"""
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            ...
            # Get course instances to use
            cur.execute("SELECT course_instance_id, course_name, faculty_name FROM course_instances LIMIT 20")
            courses = cur.fetchall()
            if not courses:
                print("No course instances found. Run setup_database.py first.")
                return
    
    print(f"Creating realistic vetrine and files for user ID: {user_id}")
    
    # Realistic vetrine data with complete information
    realistic_vetrine_data = [
    ...
    for i, vetrina_info in enumerate(realistic_vetrine_data):
    ...
    print(f"\n🎉 Successfully created {len(created_vetrine)} realistic vetrine with complete files!")
